...
class ChatGPTClient:
    ...
    async def send_message(
        ...
        # Brief pause before composing (env-tuned; defaults are light)
        await random_delay()
        ...
        submitted_text = text
        try:
            # Cursor Agent payloads are 100k+ chars. Pasting them into chatgpt.com
            # disables Send and often crashes Chromium. Skip the composer paste
            # when the proactive threshold already says the prompt is too long.
            skip_full_paste = bool(
                Config.CHATGPT_LONG_PROMPT_THRESHOLD
                and len(text) >= Config.CHATGPT_LONG_PROMPT_THRESHOLD
            )
            if skip_full_paste:
                prompt_state = "prompt-too-long"
            else:
                await self._reset_composer_for_upload()
            ...
            if (
                ...
                log.warning(
                    "Composer looks blocked, but the prompt is only %s chars; skipping file fallback",
                    len(text),
                )
                prompt_state = "ready"
            ...
            if all_attachments:
                await self._reset_composer_for_upload()
                upload_confirmed = await self._upload_files(all_attachments)
                if temporary_prompt_path and upload_confirmed is False:
                    # Never send an instruction that references a file ChatGPT
                    # did not visibly attach. Fall back to a compact prompt so
                    # the assistant receives an honest, actionable request.
                    compact = self._compact_prompt_for_composer(text)
                    log.warning(
                        "Long-prompt attachment was not confirmed; using compact prompt instead (%s chars)",
                        len(compact),
                    )
                    await self._reset_composer_for_upload()
            ...
            if auto_submitted:
                ...
                if send_ready == "disabled" and prompt_state == "prompt-too-long":
                    compact = self._compact_prompt_for_composer(text)
                    log.warning(
                        "ChatGPT did not accept the prompt file (Send still disabled); "
                        "sending a compact composer prompt instead (%s chars)",
                        len(compact),
                    )
                    await self._reset_composer_for_upload()
                    await human_type(self._page, input_selector, compact)
    ...
    async def _prompt_submission_state(self, text: str) -> PromptSubmissionState:
        ...
        if isinstance(send_button, dict) and (
            ...
            if str(state.get("composerText") or "").strip() and not state.get("hasStopButton"):
                too_long_floor = Config.CHATGPT_LONG_PROMPT_THRESHOLD or 4000
                if len(text) >= too_long_floor:
                    return "prompt-too-long"
            return "disabled"
        return "ready"
    ...
    def _compact_prompt_for_composer(text: str) -> str:
        """Last-resort short prompt when ChatGPT will not accept the attachment."""
        queries = re.findall(
        ...
        if queries:
            ...
            return (
                "The full Cursor Agent payload could not be attached. "
    ...
    async def _reset_composer_for_upload(self) -> None:
        """Clear leftover chips and ChatGPT upload dialogs from a previous retry."""
        await self._dismiss_upload_dialogs()
        await self._remove_composer_attachments()

    async def _dismiss_upload_dialogs(self) -> None:
        try:
                ...
                () => {
                    ...
                    const clickIfMatch = (el) => {
                        ...
                        for (const button of buttons) {
                            ...
                            if (/^(ok|okay|close|dismiss|got it)$/i.test(label) || label === '×') {
                                button.click();
                                hits.push(label || 'dialog-button');
                                return;
                            }
                        }
                        if (btn) {
                            btn.click();
                            hits.push('first-button');
                        }
                    };
                    document.querySelectorAll('[role="dialog"], [role="alertdialog"]').forEach(clickIfMatch);
                    document.querySelectorAll('[class*="banner"], [class*="toast"]').forEach((el) => {
                        const text = ((el.innerText || '') + '').toLowerCase();
                        if (needles.some((n) => text.includes(n))) {
                            const close = el.querySelector('button');
                            if (close) {
                                close.click();
                                hits.push('banner-close');
                            }
                        }
                    });
                    return hits;
            ...
            if dismissed:
                log.info("Dismissed upload dialogs: %s", dismissed)
        except Exception as exc:
            log.debug("Could not dismiss upload dialogs: %s", exc)

    async def _remove_composer_attachments(self) -> None:
        try:
            removed = await self._page.evaluate(
                ...
                () => {
                    ...
                    const buttons = document.querySelectorAll(
                        'button[aria-label*="Remove" i], button[aria-label*="remove" i], button[aria-label*="Delete file" i]'
                    );
                    for (const btn of buttons) {
                        names.push(btn.getAttribute('aria-label') || 'remove');
                        btn.click();
                    }
                    return names;
                }
            ...
            if removed:
                log.info("Removed composer attachments: %s", removed)
                await asyncio.sleep(0.4)
        except Exception as exc:
    ...
    async def _wait_for_send_ready(self, timeout_s: float = 20) -> str:
        ...
        while time.monotonic() < deadline:
            ...
            if isinstance(send_button, dict):
                ...
                last = "missing"
            page_text = str(state.get("composerText") or "")
            extra = str(state.get("validationText") or "")
            blob = f"{page_text} {extra}".lower()
            if "already uploaded" in blob or "failed upload" in blob or "oaiusercontent" in blob:
                await self._dismiss_upload_dialogs()
            await asyncio.sleep(0.4)
        return last if last in {"disabled", "missing"} else "disabled"
    ...
    async def _upload_files(self, file_paths: list[str]) -> bool:
        """
        ...
        try:
            ...
            log.warning("Attachment badge was not detected after upload")
            return False
