...
| `RELAY_API_KEY` | _empty_ | Optional local-only Compose token passed to `API_TOKEN`; required before network exposure. Legacy `CATGPT_API_KEY` is also accepted. |
| `RELAY_VNC_PASSWORD` | _empty_ | Optional local-only Compose GUI password. Legacy `CATGPT_VNC_PASSWORD` is also accepted. |
...
| `API_CORS_ORIGINS` | _empty_ | Comma-separated browser origins allowed to call the API; empty disables CORS. |
...
| `VNC_PASSWORD` | _empty_ | Optional local-only Web GUI/VNC password. |
